...
namespace App.Core.Reflection
{
    public class ObjectToDictionaryRegistry
    {
        ...
        private static Func<object, IDictionary<string, object>> CreateObjectToDictionaryConverter(Type itemType)
        {
            ...
            var ignoreType = typeof(AppIgnore);
            ...
            foreach (var property in itemType.GetProperties(attributes).Where(info => !info.CustomAttributes.Any(x=> x.AttributeType == ignoreType) && info.CanRead))
            {
