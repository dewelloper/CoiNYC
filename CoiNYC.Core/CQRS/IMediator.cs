namespace CoiNYC.Core.CQRS
{
    ...
    public interface IMediator
    {
        ...
        Task<TResponse> HandleAsync<TResponse>(IAsyncRequest<TResponse> request);
    ...
    {
        private static ConcurrentDictionary<long, FastInvokeHandler> _handlerCache = new ConcurrentDictionary<long, FastInvokeHandler>();
        ...
        {            
            ...
            var invokeHandler = GetInvoker<TResponse>(requestType, handlerType);

            object handler = _singleInstanceFactory(handlerType);

            return (TResponse)invokeHandler(handler, new object[] { request });
            
        }

        public FastInvokeHandler GetInvoker<TResponse>(Type requestType, Type handlerType)
        {
            long hashKey = ((long)requestType.GetHashCode() << 32) | typeof(TResponse).GetHashCode();

            FastInvokeHandler invokeHandler = null;
            if (!_handlerCache.TryGetValue(hashKey, out invokeHandler))
            {
                var methodInfo = typeof(IRequestHandler<,>)
                   .MakeGenericType(requestType, typeof(TResponse))
                   .GetMethod("Handle", new Type[] { requestType });

                invokeHandler = FastInvoker.GetMethodInvoker(methodInfo);
                _handlerCache.AddOrUpdate(hashKey, x => invokeHandler, (k, v) => v);
            }

            return invokeHandler;
        }

        public async Task<TResponse> HandleAsync<TResponse>(IAsyncRequest<TResponse> request)
        {
